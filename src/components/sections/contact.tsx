...
export function ContactSection() {
  ...
  const onSubmit = async (ev: React.FormEvent) => {
    ...
    if (!validate()) return;
    setSending(true);
    // Simulated submission (could be wired to a mailto or form service)
      ...
      <div className="relative">
        <SectionHeader
          ...
          title={
            <>
              ...
              <span className="text-brand-gradient">structures, code or research</span>
            </>
          }
          description="Use the form for technical questions, collaboration proposals, teaching invitations or feedback on the open resources."
        />
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-12">
          ...
          >
                ...
                <li key={c.label}>
                  ...
                  >
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-brand/15 text-brand-light ring-1 ring-inset ring-brand/30">
                      <PortfolioIcon name={c.icon} width={18} height={18} />
                    </span>
                    ...
                    {c.href !== "#" && (
                      <PortfolioIcon
                        ...
                        width={14}
                        height={14}
                        className="text-brand-light/40 group-hover:text-brand group-hover:translate-x-0.5 transition-all"
                      />
                    )}
                  </a>
                </li>
            ...
            <div className="mt-5 rounded-xl bg-brand/10 p-4 ring-1 ring-inset ring-brand/25">
              <p className="text-xs text-brand-light/75 leading-relaxed">
                <PortfolioIcon name="clock" width={12} height={12} className="inline mr-1 text-brand" />
                Typical response time: <span className="font-semibold text-white">2–4 business days</span>.
                For technical questions about resources, please use the forum.
              </p>
            </div>
          </motion.div>
            ...
            <div className="mt-4">
              <FormField label="Subject" htmlFor="subject" error={errors.subject}>
                <input
                  ...
                  type="text"
                  {...field("subject")}
                  placeholder="Collaboration on nonlinear analysis"
                  className="w-full rounded-md bg-ink px-3 py-2.5 text-sm text-white ring-1 ring-inset ring-brand/30 placeholder:text-brand-light/40 focus:outline-none focus:ring-2 focus:ring-brand"
                />
              </FormField>
            </div>
            ...
            <div className="mt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              >
                {sending ? (
                  ...
                  <>
                    <PortfolioIcon name="send" width={14} height={14} />
                    Send message
                  </>
                )}
              </button>
...
function FormField({
  ...
  return (
    <div>
      <label
        htmlFor={htmlFor}
        className="block text-[11px] font-mono-code uppercase tracking-[0.15em] text-brand-light/60 mb-1.5"
      >
        {label}
      </label>
      {children}
